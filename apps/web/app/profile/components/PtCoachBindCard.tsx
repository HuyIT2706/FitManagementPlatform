...
const PtCoachBindCard = ({ assignedPt, onBindSuccess }: PtCoachBindCardProps) => {
  ...
  return (
    <section
      className={`bento-card rounded-2xl p-4 sm:p-5 md:p-6 border transition-all space-y-3 ${
        isApprovedPt
    ...
    >
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        {isApprovedPt && (
          <span className="px-2.5 py-0.5 rounded-full bg-green-light/15 border border-green-light/30 text-green-light text-[10px] sm:text-[11px] font-bold flex items-center gap-1">
            <CheckCircle2 size={12} />
            Đang hoạt động
          </span>
        )}
        {isPendingPt && (
          <span className="px-2.5 py-0.5 rounded-full bg-amber-400/15 border border-amber-400/30 text-amber-400 text-[10px] sm:text-[11px] font-bold flex items-center gap-1 animate-pulse">
            <Clock size={12} />
            Chờ HLV duyệt
          </span>
        )}
      ...
      {isPendingPt ? (
        <div className="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3 pt-1">
          <input
            ...
            className="w-full sm:flex-1 border rounded-xl px-3.5 sm:px-4 py-2.5 text-xs font-extrabold outline-none tracking-wider uppercase font-mono bg-surface-bright/40 border-amber-400/20 text-amber-300/80 cursor-not-allowed select-none"
          />
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3 pt-1">
          <input
            type="text"
            name="ptCode"
            ...
            className={`w-full sm:flex-1 border rounded-xl px-3.5 sm:px-4 py-2.5 text-xs font-extrabold outline-none tracking-wider uppercase font-mono transition-all ${
              isApprovedPt
          ...
          <button
            ...
            suppressHydrationWarning
            className={`w-full sm:w-auto px-5 sm:px-6 py-2.5 font-extrabold text-xs rounded-xl transition-all shrink-0 flex items-center justify-center gap-1.5 ${
              isApprovedPt
