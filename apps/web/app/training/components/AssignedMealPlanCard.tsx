...
const AssignedMealPlanCard = ({
  ...
  const getMealConfig = (iconName?: string, mealName?: string) => {
    ...
    if (key.includes('twilight') || key.includes('sáng') || key.includes('breakfast')) {
      return {
        icon: (
          <Coffee className="w-6 h-6 sm:w-7 sm:h-7 text-amber-300 drop-shadow-[0_2px_10px_rgba(245,158,11,0.5)]" />
        ),
        badgeBg:
    ...
    if (key.includes('sunny') || key.includes('trưa') || key.includes('lunch')) {
      return {
        icon: (
          <Sun className="w-6 h-6 sm:w-7 sm:h-7 text-orange-300 drop-shadow-[0_2px_10px_rgba(249,115,22,0.5)]" />
        ),
        badgeBg:
    ...
    if (key.includes('dark') || key.includes('tối') || key.includes('dinner')) {
      return {
        icon: (
          <Moon className="w-6 h-6 sm:w-7 sm:h-7 text-indigo-300 drop-shadow-[0_2px_10px_rgba(99,102,241,0.5)]" />
        ),
        badgeBg:
    ...
    return {
      icon: (
        <Apple className="w-6 h-6 sm:w-7 sm:h-7 text-emerald-300 drop-shadow-[0_2px_10px_rgba(16,185,129,0.5)]" />
      ),
      badgeBg:
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <h3 className="font-headline-md font-bold text-lg sm:text-xl text-on-surface px-1">
        Thực đơn chỉ định hôm nay
      </h3>

      <div className="bento-card rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-6 border border-bento-border/50">
        {/* Coach Speech Bubble */}
        <div className="bg-primary/10 border border-primary/30 p-3 sm:p-4 rounded-xl sm:rounded-2xl flex gap-3 items-start">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold overflow-hidden border border-primary/40 shrink-0">
            {assignedMealPlan?.coachAvatar ? (
              ...
              <Utensils size={18} className="text-primary sm:w-5 sm:h-5" />
            )}
          </div>
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:gap-3">
          {meals.map((meal, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-start gap-3 sm:gap-4 bg-surface-bright/30 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/5 hover:border-white/10 transition-all"
              >
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl border flex items-center justify-center shrink-0 mt-0.5 ${config.badgeBg}`}
                >
                  {config.icon}
                </div>
                <div className="grow space-y-1 min-w-0">
                  <div className="flex justify-between items-center gap-2">
                    <h4 className="font-bold text-on-surface text-sm sm:text-base truncate">{meal.name}</h4>
                    <span className="text-green-light font-bold text-xs shrink-0">{meal.kcal} kcal</span>
                  </div>
                  <p className="text-[11px] sm:text-xs text-on-surface-variant whitespace-pre-line leading-relaxed font-medium">
                    {meal.description}
                  </p>
                </div>
              </div>
        ...
        <div className="pt-1">
          <div className="bg-surface-bright/40 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border border-white/5 space-y-2">
            ...
            <div className="h-2 sm:h-2.5 w-full bg-surface-dim rounded-full overflow-hidden">
              <div
