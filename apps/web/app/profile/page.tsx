      ...
      <main className="px-4 md:px-10 pt-4 max-w-7xl mx-auto flex flex-col gap-4">
        ...
        <section className="bento-card rounded-2xl p-5 md:p-6 border border-primary/30 bg-primary/5 space-y-3">
          ...
          <p className="text-xs text-on-surface-variant font-medium">
            Nhập Mã PT duy nhất của Coach (ví dụ: <strong className="text-primary">PT-HUY066</strong>) hoặc Mã Mời từ Gmail để kết nối 1-1.
          </p>

          <form
            onSubmit={(e) => {
              e.preventDefault();
              const input = (e.currentTarget.elements.namedItem("ptCode") as HTMLInputElement).value;
              if (!input) {
                toast.error("Vui lòng nhập Mã PT!");
                return;
              }
              apiClient
                .post("/pt/students/bind", { ptCodeOrInviteCode: input })
                .then((res) => {
          ...
          >
            <input
              ...
              className="w-full sm:flex-1 bg-surface-bright border border-white/10 rounded-xl px-4 py-2.5 text-xs text-on-surface font-extrabold focus:border-primary outline-none tracking-wider uppercase"
            />
            <button
              type="submit"
