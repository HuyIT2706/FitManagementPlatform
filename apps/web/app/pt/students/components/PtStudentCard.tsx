...
const PtStudentCard = ({ student }: PtStudentCardProps) => {
  ...
  return (
    <div className="bento-card rounded-xl sm:rounded-2xl p-3.5 sm:p-5 border border-outline-variant/30 space-y-3.5 sm:space-y-4 relative group hover:border-primary/50 transition-all">
      ...
      <div className="pt-2.5 sm:pt-3 border-t border-slate-200 dark:border-white/5 flex flex-wrap sm:flex-nowrap items-center justify-between gap-2">
        <Link
          href={`/pt/students/${student.id}`}
          className="flex-1 min-w-[150px] bg-primary/10 text-primary-dark dark:text-primary hover:bg-primary/20 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors border border-primary/20 text-center"
        >
